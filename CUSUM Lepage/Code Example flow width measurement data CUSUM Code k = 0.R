...
generate_plot_df <- function(phase_2, u,k=0) {
  ...
  for( t in 1:nrow(Phase_2)){
    if(t==1){
      plot_df[t,"C_C"] = max(0,0+(plot_df[t,"SC"]-1)-k)
      ...
      plot_df[t,"C_C"] = max(0,plot_df[t-1,"C_C"]+(plot_df[t,"SC"]-1)-k)
...
plot_df <- generate_plot_df(Phase_2, u,k=0)
print(plot_df)
...
ARL_graph <- create_plot(plot_df,"C_VWAB",0,23.15,bquote(CL["V,A"]))
print(ARL_graph)

ARL_graph <- create_plot(plot_df,"C_VWM",0,22.45,bquote(CL["V,M"]))
print(ARL_graph)

ARL_graph <- create_plot(plot_df,"C_WRSAB",0,25.5,bquote(CL["W,A"]))
print(ARL_graph)

ARL_graph <- create_plot(plot_df,"C_C",0,11.9,bquote('CUSUM Cucconi'))
print(ARL_graph)


# Save the final data set
output_file <- "Example Flow Width measurement Statistics for n=5 and k = 0 .csv"
