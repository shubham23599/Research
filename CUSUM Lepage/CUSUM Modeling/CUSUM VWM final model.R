...
formula <- sqrt(H) ~ sqrt(k) + sqrt(m) + sqrt(n) + sqrt(ARL)
...
test_predictions <- predict(model, newdata = test_data)**2
...
final_results <- data.frame(
  ...
  Value = c(NA,  # p-values for assumption tests
            ...
            paste0("k: ", round(vif_values["sqrt(k)"], 2),
                   ", m: ", round(vif_values["sqrt(m)"], 2),
                   ", n: ", round(vif_values["sqrt(n)"], 2),
                   ", ARL: ", round(vif_values["sqrt(ARL)"], 2))
  ),
