# Shewhart Max Chart based on V-M Chart Simulation
  ...
  for(d in 1:length(delta)){
    ...
    for (j in 1:Simulations) {
      ...
      while (S <= H & RL[j] <= 5000) {
        ...
        S1 <- abs((XN - 0) / sqrt(s_1)) 
        S2 <- abs((sum((Ranks-1/2*(N+1))^2*Indicator)-m2)/sqrt(s2))
        S <- max(S1,S2)
        RL[j] <- RL[j] + 1
      }
    }
...
write.csv(data_arl, "L_Shew_Max_ARL_V_M.csv", row.names = FALSE)
