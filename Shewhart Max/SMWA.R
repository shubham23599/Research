...
# Calculate mean and variance for WRS test
m1 <- 1/2 * (n * (N + 1))
s1 <- 1/12 * (m * n * (N + 1))
  ...
  for(d in 1:length(delta)){
    ...
    for (j in 1:Simulations) {
      ...
      while (S <= H & RL[j] <= 5000) {
        ...
        Ranks <- rank(c(u, v))
        S1 <- abs((sum(Ranks*Indicator) - m1) / sqrt(s1)) 
        S2 <- abs((sum(abs((Ranks - 1/2 * (N + 1))) * Indicator) - m2) / sqrt(s2))
        S <- max(S1,S2)
        RL[j] <- RL[j] + 1
      }
    }
...
write.csv(data_arl, "L_Shew_Max_ARL_W_AB.csv", row.names = FALSE)
