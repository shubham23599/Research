  ...
  for (j in seq_len(Simulations)) {
    ...
    if (class == "none") {
      if (length(test_sample) >= 2) {
        ...
        p2b <- mood.test(u, combined)$p.value
...
run_lepage_simulation_batch <- function(
    ...
    results_excel = "All_Results_EC.xlsx",
    summary_excel = "Summary_Final_EC.xlsx",
...
run_lepage_simulation_batch(
  ...
  results_excel = "EC_Results_All.xlsx",
  summary_excel = "EC_Summary_All.xlsx"
)
