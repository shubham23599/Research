...
run_lepage_simulation_once <- function(
  ...
  for (j in seq_len(Simulations)) {
    ...
    p1 <- VanWaerdenTest(list(last_v, u))$p.value
    ...
    if (class == "none") {
      if (length(test_sample) >= 2) {
        ...
        p1b <- VanWaerdenTest(list(combined, u))$p.value
...
run_lepage_simulation_batch(
  ...
  results_excel = "ELVM_Results_All.xlsx",
  summary_excel = "ELVM_Summary_All.xlsx"
)
